...
3. **Multi-omic integration** (multi-method approach per Communications Biology 2025 benchmark, doi:10.1038/s42003-025-08515-9):
   - *Primary biomarker detection*: **MiMeJF** (Metabolites 2025, doi:10.3390/metabo15010051) — IBD-validated CMTF implementation that identified B. vulgatus + acylcarnitines in IBD cohorts. Provides ready-to-use coupled matrix and tensor factorization for joint microbiome-metabolome analysis. CMTF consistently outperformed traditional methods in both dimension reduction AND biomarker detection in the 19-method benchmark (Commun Biol 2025). MiMeJF handles coupled microbiome-metabolome matrices natively and is robust to noise typical of clinical IBD cohorts.
   - *Supervised phenotype-guided discovery*: **ACMTF-R** (PLOS One 2026, doi:10.1371/journal.pone.0339650) — supervised extension of ACMTF with regression step. Simultaneously decomposes multi-way data while capturing outcome-associated variation. Use for phenotype-guided factor discovery — directly identify subtype-specific factors linked to disease behavior, treatment response, or calprotectin levels.
   - *Transfer learning for small samples*: **CoMMiT** (arXiv 2025, 2506.24013) — within-cohort transfer learning for microbiome-metabolome interactions. Transfers knowledge from auxiliary metabolites without external datasets, minimizing negative transfer. Use for validation cohorts lacking paired metabolomics — enables metabolite prediction from metagenomics alone + de-biased p-values for interactions.
