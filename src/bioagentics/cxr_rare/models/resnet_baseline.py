"""ResNet-50 baseline for CXR long-tail multi-label classification.

ImageNet-pretrained ResNet-50 with multi-label sigmoid output head.
Secondary baseline for comparison alongside DenseNet-121.
"""
...
class ResNet50CXR(nn.Module):
    """ResNet-50 with multi-label classification head.
...
def train_resnet_baseline(
    ...
    """Train ResNet-50 baseline with standard BCE."""
    ...
    model = ResNet50CXR(num_classes=NUM_CLASSES, pretrained=True, dropout=0.2)
    ...
    trainer = CXRTrainer(
        ...
        experiment_name="resnet50_bce_baseline",
    )
    ...
    if all_labels:
        ...
        return evaluate_and_save(y_true, y_score, output_dir, "resnet50_bce_baseline")

    return {}


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train ResNet-50 CXR baseline")
    ...
    train_resnet_baseline(args.data_dir, args.output_dir, config)
