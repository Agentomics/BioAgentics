...
(SMARCA4-deficient vs intact) across all ~18,000 genes in CRISPRGeneEffect.
Identifies cancer-type-specific and universal SL hits.
Also checks top hits for mutation class specificity (Class 1 vs Class 2).

Usage:
    uv run python -m smarca4_pancancer_sl_atlas.03_genomewide_sl_screen
"""
...
OUTPUT_DIR = REPO_ROOT / "data" / "results" / "smarca4-pancancer-sl-atlas" / "phase3"
...
KNOWN_SL_TARGETS = {
    "SMARCA2", "ARID1A", "ARID1B", "PBRM1", "EZH2",
    "BRD2", "BRD4", "HDAC1", "HDAC2",
    "ATR", "PARP1", "HSP90AA1", "CDK4", "CDK6",
}
...
def screen_cancer_type(
    ...
    for gene in crispr_cols:
        ...
        if len(def_vals) < MIN_SAMPLES or len(int_vals) < MIN_SAMPLES:
            continue
...
def plot_volcano(
    ...
    ax.set_xlabel("Cohen's d (SMARCA4-deficient vs intact)")
...
def main() -> None:
    ...
    for cancer_type in qualifying:
        ...
        n_def = (ct_data["smarca4_status"] == "deficient").sum()
        n_intact = (ct_data["smarca4_status"] == "intact").sum()
        print(f"  Screening {cancer_type} ({n_def} deficient, {n_intact} intact)...")
    ...
    # Universal SL genes
    if len(sl_hits) > 0 and "category" in sl_hits.columns:
    ...
    if len(universal) > 0:
        ...
        universal_genes = pd.DataFrame(columns=["gene", "n_cancer_types", "mean_cohens_d", "min_fdr", "cancer_types"])
    ...
    for _, row in novel_summary.head(20).iterrows():
        in_swisnf = " [SWI/SNF]" if row.get("is_swisnf", False) else ""
        print(f"    {row['gene']}: mean d={row['mean_cohens_d']:.3f}, "
