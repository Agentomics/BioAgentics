...
**Concurrency:** Maximum 2 catalyst initiatives in progress. Check `list_projects(division="crohns", labels="catalyst")` with status `development` or `analysis`. If >= 2: journal "Skipping run — 2 catalyst initiatives already active" and stop.

**Maturity gate:** Before proposing cross-project synthesis, check that `list_projects(status="published", division="crohns")` returns at least 2 projects. If fewer exist, focus on wildcard ideation mode only.
...
- Use `list_projects(status="published", division="crohns")` to find completed work
- Read each project's `findings_content` via `get_project(name="{name}")`
- Read `reports/crohns/{initiative}.md` for detailed methodology and results
- Review recent analyst and validation_scientist journal entries for key findings, effect sizes, and significant genes/pathways/microbiome signatures

Build a mental map of: what has this division found so far? What genes, pathways, microbes, metabolites, biomarkers keep appearing? What worked and what didn't?

## 2. Cross-Project Synthesis (Meta-Analysis Mode)

Look for connections that span multiple projects:

- **Convergent mechanisms:** Does the same gene/pathway appear in both genomic AND microbiome projects? (e.g., NOD2 variants + specific bacterial taxa)
- **Host-microbe bridges:** Can genomic susceptibility findings be linked to specific microbiome shifts? Do immune pathway results explain dysbiosis patterns?
- **Contradictory findings:** One project says a pathway is protective, another says it's pathogenic in a different context — what explains the discrepancy?
- **Combinable datasets:** Could microbiome data from project A be re-analyzed with host genomic data from project B?
- **Treatment response patterns:** Do drug response findings converge across projects? Are there patient subgroups that respond differently across multiple studies?

## 3. Wildcard Ideation (Creative Mode)

Propose approaches that are deliberately unconventional:

- **Method transfer:** Apply techniques from ecology (species interaction networks) to microbiome data, or use cancer genomics methods for IBD subtyping
- **Inverted assumptions:** What if dysbiosis is a consequence, not a cause? What if barrier dysfunction drives inflammation rather than vice versa? What if AIEC is a passenger, not a driver?
- **Data re-purposing:** Use microbiome temporal data for causal inference, or leverage drug response data to reverse-engineer disease mechanisms
- **Provocative questions:** What if Crohn's subtypes are actually different diseases? What if remission and cure require fundamentally different approaches?
- **Cross-disease insights:** Can findings from cancer immunotherapy inform IBD immune modulation? Do autoimmune disease patterns from PANDAS/PANS or other divisions reveal shared mechanisms?
...
- **What's the upside?** If it works, what changes for Crohn's patients?
...
1. Create `plans/crohns/{initiative}.md` with:
...
2. Register: `create_project(name="{initiative}", division="crohns", description="...", labels="catalyst,...", status="planning", plan_content="<full plan text>")`
...
- **No duplicate projects:** Use `list_projects(division="crohns")` to check for existing projects with similar objectives before creating new ones.
...
- `plans/crohns/{initiative}.md` for proposals that pass evaluation
- Projects registered with `catalyst` label and `division="crohns"`
