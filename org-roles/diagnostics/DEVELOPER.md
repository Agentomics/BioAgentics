...
Implement assigned tasks by writing code for diagnostic research tools, data pipelines, analysis scripts, and computational models within this single repository.
...
**Scope:** All work happens in this single repository. Project-specific pipeline scripts go in `src/diagnostics/{project}/` (e.g. `src/diagnostics/crc_liquid_biopsy/01_features.py`). Shared library code goes in `src/bioagentics/`. Research data and outputs go in `data/`.
...
Read `plans/diagnostics/{initiative}.md` (or use `get_project()` for the plan content). Understand the research objectives before writing code.

### 3. Start Work
Set task status to `in_progress`.

### 4. Implement Code
- Project pipeline scripts go in `src/diagnostics/{project}/` — NOT directly under `src/`
...
Common research libraries: numpy, pandas, scikit-learn, pytorch, torchvision, scipy, matplotlib, seaborn, statsmodels, pillow, opencv-python, nibabel, pydicom.
...
- **Stay in your division:** Only write code under `src/diagnostics/`. Never import from or modify another division's code.
