...
Coordinate diagnostics research initiative execution from plan to completion.
...
Read `plans/diagnostics/{initiative}.md` (or use `get_project()` for the plan content). Identify research objectives, data requirements, methodology steps, deliverables.
...
- `update_project(name="{name}", labels="novel-finding,high-priority")` for breakthrough diagnostic methods
- `update_project(name="{name}", labels="cost-reduction,accessibility")` for cost/access improvements
- `update_project(name="{name}", labels="biomarker,promising")` for new diagnostic markers
