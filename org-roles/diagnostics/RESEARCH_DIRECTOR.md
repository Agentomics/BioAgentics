...
**Concurrency:** Maximum 7 research initiatives in progress. Check `list_projects(status="development", division="diagnostics")` + `list_projects(status="analysis", division="diagnostics")` and count. If >= 7: journal "Skipping run — 7 initiatives already active" and stop.
