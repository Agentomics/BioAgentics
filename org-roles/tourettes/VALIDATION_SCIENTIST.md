...
## 1. Retrieve Tasks
Query tasks assigned to `validation_scientist` with status `pending` and `division="tourettes"`.

## 2. Start Review
Set task status to `in_progress`.

## 3. Read Research Plan
Read `plans/tourettes/{initiative}.md` (or use `get_project()` for the plan content) to understand the intended methodology and success criteria.
